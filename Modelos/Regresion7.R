#REGRESION CON GRUPOS DE POPULARIDAD DE RANGO 20 Y CON LAS 2 VARIABLES
...
data<-dbGetQuery(con, "SELECT popularity, danceability, instrumentalness FROM cancion")

data$group <- cut(data$popularity, c(-1, 20, 40, 60, 80, 100), labels = c('group0','group1','group2','group3','group4'))
data$group <- as.numeric(data$group)

formula<- data$group ~ data$danceability + data$instrumentalness
...
EMPaPriori #34.14% de error medio "a priori"
